...
public final class RingBufferImpl<E> implements RingBuffer<E> {
     ...
     */
    RingBufferImpl(EventFactory<E> eventFactory,
                   Sequencer sequencer) {
    ...
    public static <E> RingBuffer<E> createMultiProducer(EventFactory<E> factory,
        ...
        return new RingBufferImpl<E>(factory, sequencer);
    }
     ...
     * After calling {@link RingBufferImpl#next()} use this call to get hold of the
     * preallocated event to fill with data before calling {@link RingBufferImpl#publish(long)}.</p>
     * <p/>
    ...
    /**
     * @deprecated Use {@link RingBufferImpl#get(long)}
     */
    @Override
    @Deprecated
     ...
     * @see RingBufferImpl#publish(long)
     * @see RingBufferImpl#get(long)
     */
    @Override
    public long next() {
    ...
    /**
     * The same functionality as {@link RingBufferImpl#next()}, but allows the caller to claim
    ...
    /**
     * The same functionality as {@link RingBufferImpl#tryNext()}, but allows the caller to attempt
     ...
     * that a call to {@link RingBufferImpl#next()} will not block.  Especially true if this
