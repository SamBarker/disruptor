 ...
 * an event being exchanged between event producer and {@link com.lmax.disruptor.EventProcessor}s.
 *
 * @param <E> implementation storing the data for sharing during exchange or parallel coordination of an event.
 */
public final class BatchPublicationRingBuffer<E> implements RingBuffer<E>
{

    private final RingBufferImpl<E> ringBuffer;
     ...
     */
    BatchPublicationRingBuffer(EventFactory<E> eventFactory,
                               Sequencer sequencer)
    {
        ringBuffer = new RingBufferImpl<E>(eventFactory, sequencer);
    }

    /**
     * Create a new multiple producer RingBuffer with the specified wait strategy.
     *
     * @see com.lmax.disruptor.MultiProducerSequencer
     * @param factory used to create the events within the ring buffer.
     * @param bufferSize number of elements to create within the ring buffer.
     * @param waitStrategy used to determine how to wait for new elements to become available.
     * @throws IllegalArgumentException if bufferSize is less than 1 and not a power of 2
     */
    public static <E> BatchPublicationRingBuffer<E> createMultiProducer(EventFactory<E> factory,
                                                        int             bufferSize,
                                                        WaitStrategy    waitStrategy)
    {
        MultiProducerSequencer sequencer = new MultiProducerSequencer(bufferSize, waitStrategy);

        return new BatchPublicationRingBuffer<E>(factory, sequencer);
    }

    /**
     * Create a new multiple producer RingBuffer using the default wait strategy  {@link com.lmax.disruptor.BlockingWaitStrategy}.
     *
     * @see com.lmax.disruptor.MultiProducerSequencer
     * @param factory used to create the events within the ring buffer.
     * @param bufferSize number of elements to create within the ring buffer.
     ...
     */
    public static <E> BatchPublicationRingBuffer<E> createMultiProducer(EventFactory<E> factory, int bufferSize)
    {
     ...
     *
     * @see com.lmax.disruptor.SingleProducerSequencer
     * @param factory used to create the events within the ring buffer.
     * @param bufferSize number of elements to create within the ring buffer.
     * @param waitStrategy used to determine how to wait for new elements to become available.
     * @throws IllegalArgumentException if bufferSize is less than 1 and not a power of 2
     */
    public static <E> BatchPublicationRingBuffer<E> createSingleProducer(EventFactory<E> factory,
                                                         int             bufferSize,
                                                         WaitStrategy    waitStrategy)
    {
    ...
    /**
     * Create a new single producer RingBuffer using the default wait strategy  {@link com.lmax.disruptor.BlockingWaitStrategy}.
     *
     * @see com.lmax.disruptor.MultiProducerSequencer
     * @param factory used to create the events within the ring buffer.
     * @param bufferSize number of elements to create within the ring buffer.
     * @throws IllegalArgumentException if <tt>bufferSize</tt> is less than 1 and not a power of 2
     */
    public static <E> BatchPublicationRingBuffer<E> createSingleProducer(EventFactory<E> factory, int bufferSize)
    {
     ...
     * @throws IllegalArgumentException if bufferSize is less than 1 and not a power of 2
     */
    public static <E> BatchPublicationRingBuffer<E> create(ProducerType    producerType,
     ...
     * After calling {@link BatchPublicationRingBuffer#next()} use this call to get hold of the
     * preallocated event to fill with data before calling {@link BatchPublicationRingBuffer#publish(long)}.</p>
     *
     * <p>Secondly use this call when consuming data from the ring buffer.  After calling
     * {@link com.lmax.disruptor.SequenceBarrier#waitFor(long)} call this method with any value greater than
     * that your current consumer sequence and less than or equal to the value returned from
     * the {@link com.lmax.disruptor.SequenceBarrier#waitFor(long)} method.</p>
     *
    ...
    /**
     * @deprecated Use {@link BatchPublicationRingBuffer#get(long)}
     */
    @Deprecated
     ...
     * }
     * </pre>
     * @see BatchPublicationRingBuffer#publish(long)
     * @see BatchPublicationRingBuffer#get(long)
    ...
    /**
     * The same functionality as {@link BatchPublicationRingBuffer#next()}, but allows the caller to claim
     ...
     * @see BatchPublicationRingBuffer#get(long)
     * @return The next sequence to publish to.
     * @throws com.lmax.disruptor.InsufficientCapacityException if the necessary space in the ring buffer is not available
     */
    ...
    /**
     * The same functionality as {@link BatchPublicationRingBuffer#tryNext()}, but allows the caller to attempt
    ...
    {
        return ringBuffer.claimAndGetPreallocated(sequence);
    }
    ...
    {
        return ringBuffer.isPublished(sequence);
    }
    ...
    {
        return ringBuffer.removeGatingSequence(sequence);
    }
    ...
    {
        return ringBuffer.newBarrier(sequencesToTrack);
    }
     ...
     * that a call to {@link BatchPublicationRingBuffer#next()} will not block.  Especially true if this
    ...
    public void publishEvents(EventTranslator<E>[] translators, int batchStartsAt, int batchSize) {
        ...
        final long finalSequence = ringBuffer.next(batchSize);
    ...
    public boolean tryPublishEvents(EventTranslator<E>[] translators, int batchStartsAt, int batchSize) {
        ...
        try {
            final long finalSequence = ringBuffer.tryNext(batchSize);
            translateAndPublishBatch(translators, batchStartsAt, batchSize, finalSequence);
            return true;
    ...
    private boolean invalidatesBounds(final EventTranslator<E>[] translators, final int batchStartsAt, final int batchSize) {
        return batchStartsAt < 0 || batchSize <= 0 || batchOverRuns(translators, batchStartsAt, batchSize) || batchSize > ringBuffer.getBufferSize();
    }

    private <A> boolean invalidatesBounds(final A[] arg0, final int batchStartsAt, final int batchSize) {
        return batchStartsAt < 0 || batchSize <= 0 || batchOverRuns(arg0, batchStartsAt, batchSize) || batchSize > ringBuffer.getBufferSize();
    }
    ...
    private <A, B> boolean invalidatesBounds(final A[] arg0, final B[] arg1, final int batchStartsAt, final int batchSize) {
        return batchStartsAt < 0 || batchSize <= 0 || batchOverRuns(arg0, batchStartsAt, batchSize) ||
                batchOverRuns(arg1, batchStartsAt, batchSize) || batchSize > ringBuffer.getBufferSize();
    }
    ...
    private <A, B, C> boolean invalidatesBounds(final A[] arg0, final B[] arg1, final C[] arg2, final int batchStartsAt, final int batchSize) {
        return batchStartsAt < 0 || batchSize <= 0 || batchOverRuns(arg0, batchStartsAt, batchSize) || batchOverRuns(arg1, batchStartsAt, batchSize)
                || batchOverRuns(arg2, batchStartsAt, batchSize) || batchSize > ringBuffer.getBufferSize();
    }

    private boolean invalidatesBounds(final int batchStartsAt, final int batchSize, final Object[][] args) {
        return batchStartsAt < 0 || batchSize <= 0 || batchOverRuns(args, batchStartsAt, batchSize) || batchSize > ringBuffer.getBufferSize();
    }
